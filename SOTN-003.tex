...
\begin{figure}
    ...
    \caption{Comparison of the observing condition for pattern-visible exposures vs. pattern-invisible exposures.}
...
Rather, the reason for the unpredictable pattern visibility is coming from the optical system itself.

The basis of this investigation relied upon the assumption that, when applying a perturbation to a particular degree of freedom, the perturbation is starting at or near 0. 
Meaning, if applying a perturbation of -2000.00 $\mu$m to M1M3 B5, the final position of that bending mode should be at or near -2000.00 $\mu$m. 
If the bending mode is not beginning at or near 0, the final perturbation being applied to the telescope could be anything. 
Part of the reason to select such large perturbations for the degrees of freedom was to account for and minimize this possibility. 
However, it is likely that this assumption is generally not valid and interfering with the ability to isolate a given degree of freedom for study. 
Initially, the trend that pattern-visibility was only present in hexapod DOFs was thought to be because they are less subtle than the bending modes and typically perturbed by larger amounts. 
It is more likely that these exposures are more commonly pattern-visible because it is easier to identify the `0' of these DOFs. 
Generally, it is more straightforward to assume a null position for the LSST Camera on the $z$-axis than the null position of a high order bending mode correcting for gravitational sag underneath the M1M3 Mirror Monolith. 

Ellipticity has previously been noted to infrequently agree with the predicted values while on-sky, and it is possible that the failure of the `near-0' assumption is the reason for this discrepancy. 
While the source of the unpredictable ellipticity behavior on the Rubin Observatory originates within the optical system of the telescope, the ability to further study and correct for it requires establishing a relationship between the behavior and the DOFs. 
Further work would be required to account for the nonlinear nature of ellipticity and the unreliability of the `near-0' assumption. 
