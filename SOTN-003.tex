...
In order to analyze this data, we compared the on-sky images to models created using \texttt{StarSharp}: a simulation package which allows us to simulate the focal plane based on input perturbations to any of the degrees of freedom. 
...
Below, Figure \ref{fig:example_image} shows on-sky data next to the \texttt{StarSharp} model for this investigation.

\begin{figure}
    \centering
    \includegraphics[width=1.0\linewidth]{2025041700635_M2_B1.png}
    \caption{Comparison between the on-sky second moment information measured using the PSF information from stars in the in-focus image and the modeled image built using \texttt{StarSharp}. This data is for the M2B1 bending mode. The in-focus image was taken on April 17th, 2025.}
    \label{fig:example_image}
\end{figure}

The exposures for this investigation were examined by eye and formatted into an \texttt{astropy.table.Table} object. 
...
Some of this data is reflected in Figure \ref{fig:conditions}, where we can see that there is not any signification correlation between the observing conditions and the visibility of the second moment patterns. Seeing measurements recorded by the DIMM, wind speeds, or even the total PSF do not seem to influence the visibility of these patterns. 
...
Comparing the second moment information from on-sky data to the models, such as in Figure \ref{fig:example_image}, it is clear that the patterns are not typically recoverable. For the 117 exposures of the investigation, only 15 were considered to reflect the expected pattern for manipulating a given degree of freedom. 
