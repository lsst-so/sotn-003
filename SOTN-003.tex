...
The offsets applied to the selected degrees of freedom are listed in Tables \ref{tab:hexapods} and \ref{tab:bending}.
In total, the data for this investigation is comprised of 117 exposures. 

\begin{table*}
\centering
\caption{This table is an outline of hexapod perturbations applied to the telescope. For perturbations to the $x$, $y$, or $z$ components, the units are in microns. For perturbations to the hexapod tilts ($rx$ or $ry$) the units are in degrees.}
\label{tab:hexapods}
\begin{tabular}{c|c|c}
\hline
Hexapod $z$ &
...
\begin{table*}
\centering
\caption{Bending mode perturbations applied: (EDIT THIS TABLE FORMATTING)}
\label{tab:bending}
\begin{tabular}{c|c}
\hline
M2 Bending Modes &
...
In order to analyze this data, we compared the on-sky images to images modeled using \texttt{StarSharp}: a simulation package which allows us to model the focal plane based on input perturbations to any of the degrees of freedom. 
...
Columns of the \texttt{Table} object consisted of information pulled out of the \texttt{ConsDB} and \texttt{ASDF} metadata files processed from the Butler.
