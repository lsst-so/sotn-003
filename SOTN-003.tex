...
It is much more likely that these exposures are more commonly pattern-visible because it is easier to identify the ``0' of these DOFs. 
Generally, it is more straightforward to identify a null position for the LSST Camera on the $z$-axis than the null position of a high order bending mode correcting for gravitational sag underneath the M1M3 Mirror Monolith. 

Ellipticity has previously been noted to infrequently agree with the predicted values while on-sky, and it is possible that the failure of the ``near-0' assumption is the reason why. 
